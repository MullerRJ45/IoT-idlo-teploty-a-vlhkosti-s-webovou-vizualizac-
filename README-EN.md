...
First, you need to prepare an SD card or another supported storage medium. Then download the Raspberry Pi Imager app from the official website:
...
Once everything is set, start writing the system to the medium. When it is done, insert the SD card into the Raspberry Pi and power it on. From there, just follow the initial setup of Raspberry Pi OS.

After the system boots and the desktop appears, it is a good idea to update everything first:

```bash
sudo apt update && sudo apt full-upgrade -y
```

You will also want to enable SSH so you can manage the Raspberry Pi remotely:
```bash
...
- 28 — DNC
![Wiring](image1.png)
I found the sensor address using:
```bash
sudo i2cdetect -y 1
```
...
with SMBus(1) as bus:
    ...
    print("Raw HEX data:", hex_data)
```
If you are getting an error, double-check the sensor address and try again.
...
Start InfluxDB with `sudo systemctl start influxdb`, verify it is running with `sudo systemctl status influxdb`, and stop it if needed with `sudo systemctl stop influxdb`. If the service is active, log into the web interface at `http://localhost:8086`. That works when accessing it directly from the Raspberry Pi. If you are accessing it from another device, make sure both are on the same network. You can find the Raspberry Pi IP address by running `hostname -I` on it, then enter `http://(YourIP):8086` in your browser.
...
I saved the web app backend in a folder called `my_dashboard` and the frontend in a subfolder called `templates`.
...
## 6. Automation and Autostart on Boot
